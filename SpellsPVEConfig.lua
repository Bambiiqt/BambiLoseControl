...
local function SetTabs(frame, numTabs, ...)
	...
	for i = 1, numTabs do
		...
		if (i == 1) then
			tab:SetPoint("TOPLEFT", UISpellsPVEConfig, "BOTTOMLEFT", 6, 7);
			rowCount = 1
		else
			if rowCount <= 9 then
...
local function CreateMenu()
	...
	UISpellsPVEConfig.ScrollFrame = CreateFrame("ScrollFrame", nil, UISpellsPVEConfig, "ScrollFrameTemplate");
		...
		local spellID, prio, instanceType, zone, duration, customname, _, _ = unpack(L.spells[i+1][l][x])
			if (spellID) then
				...
				if spellCheck then
					...
					spellCheck.icon.check = spellCheck
					local aString = spellID
					local aString1, aString2
					prio = L[prio] or prio
					if type(spellID) == "number" then
						local name = GetSpellInfo(spellID)
						if (instanceType ==  "arena" or instanceType == "pvp") then
							if duration then
								if name then  aString1 = substring(GetSpellInfo(spellID), 0, 14)..": "..substring(prio, 0, 5)..": "..math.floor(duration) else aString1 = "SPELL REMOVED: "..spellID end
								local aString2 = " ("..instanceType..")"
								local cutString1 = substring(aString1, 0, 23);
								local cutString2 = substring(aString2, 0, 23);
								local aString3 = cutString1.."\n"..cutString2
